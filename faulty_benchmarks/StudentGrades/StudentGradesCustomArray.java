...
import java.util.*;public class StudentGradesCustomArray {
 ...
 public static  String[] FlatMap1(String result){
        ...
        String[] ans=flatMap5.apply( result);
return ans;
}
 public static ArrayList< map4> Map1(String[] result){
        int callersLineNumber = Thread.currentThread().getStackTrace()[1].getLineNumber();
...
for ( map3 results: result){if (filter1.apply(results._1(),results._2()))ans.add(results );}
return ans;
}
}